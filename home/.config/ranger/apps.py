	...
	def app_default(self, c):
		...
		if f.extension is not None:
			if f.extension in ('pdf', ):
				return self.either(c, 'fbpdf', 'evince', 'zathura', 'apvlv')
			if f.extension == 'djvu':
				return self.either(c, 'evince')
			if f.extension in ('xml', ):
				return self.either(c, 'editor')
			if f.extension in ('html', 'htm', 'xhtml'):
				return self.either(c, 'w3m', 'firefox', 'opera', 'jumanji', 'luakit', 'elinks', 'lynx')
			if f.extension == 'swf':
		...
		if f.video or f.audio:
			if f.video:
				#c.flags += 'd'
				return self.either(c, 'mplayer2', 'mplayer', 'smplayer', 'totem', 'nvlc')
			if f.audio:
				return self.either(c, 'nvlc', 'mplayer')

		if f.image:
			if c.mode in (11, 12, 13, 14):
				return self.either(c, 'set_bg_with_feh')
			else:
				return self.either(c, 'mirage', 'fbi', 'sxiv', 'feh', 'eog')
